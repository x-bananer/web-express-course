...
const listCats = async () => {
	const [rows] = await promisePool.query(`
		...
		FROM wsk_cats
		LEFT JOIN wsk_users ON wsk_cats.owner_id = wsk_users.id
	`);
...
const updateCat = async (updatedCat) => {
	...
	const sql = promisePool.format(`UPDATE wsk_cats SET ? WHERE id = ?`, [
		cat,
		id,
	]);
